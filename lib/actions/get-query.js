...
const flip = obj => Object.entries(obj).reduce((obj, [key, value]) => ({ ...obj, [value]: key }), {})

// Function to test whether object has nested objects
const testForNested = (obj) => {
  const query = obj
...
module.exports = (request, response, config) => {
  ...
  // Map query keys to mongodb ?
  function mapQueryKeys (obj) {
    // { _page: 1, _limit: 5 } TO
    // { page: 1, limit: 5 }
    const { queryKeys } = config // { page: '_page', ... }
    const queryKeysFlipped = flip(queryKeys) // { '_page': 'page', ... }
    // const mappedQuery = Object.keys(query).map((value, key) => _queryKeys[key])

    let mappedQuery = {}
    for (var key in obj) {
      const newKey = queryKeysFlipped[key] // key = '_page' => 'page'
      mappedQuery[newKey] = obj[key]
    }
  ...
  // ORIGINAL ?query={"key":"value"}&fields={"name":1,"surname":1}&limit=10&skip=2&snapshot=1&sort={"name":-1}&hint=index_name
  // JSON-SERVER ?_page=1&_limit=5&_sort=fullName&_order=asc
  function buildQueryOptions (request) {
    ...
    const { query } = request
    query['_sort'] = `{"${query['_sort']}":${query['_order'] === 'asc' ? 1 : -1}}` // DIRTY FIX

    const validTags = Object.keys(query)
      .filter(o => names.indexOf(o) >= 0)

    const tags = mapQueryKeys(query)

    // TODO ?
    // return tags.map(o => {
    //   return (needParse.indexOf(o) >= 0)
    //     ? JSON.parse(request.query[o])
    //     : request.query[o]
    // })
